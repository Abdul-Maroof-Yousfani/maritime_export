...
@section('content')
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="well">
                            <div class="row">
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <span class="subHeadingLabelClass">Create Cash Sale Voucher Form</span>
                                </div>
                            </div>
                            <div class="lineHeight">&nbsp;</div>
                            <div class="row">
                                <?php echo Form::open(array('url' => 'sad/addCashSaleVoucherDetail?m='.$m.'','id'=>'addCashSaleVoucherDetail'));?>
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <div class="panel">
                                        <div class="panel-body">
                                            <div class="row">
                                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                    <input type="hidden" name="cashSaleSection[]" class="form-control requiredField" id="cashSaleSection" value="1" />
                                                </div>
                                            </div>
                                            <div class="row">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                    ...
                                                    <select class="form-control requiredField" name="credit_acc_id_1" id="credit_acc_id_1">
                                                        ...
                                                        @foreach($creditAccounts as $key => $y1)
                                                ...
                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                    <label class="sf-label">Debit Account Head</label>
                                                    <span class="rflabelsteric"><strong>*</strong></span>
                                                    <select class="form-control requiredField" name="debit_acc_id_1" id="debit_acc_id_1">
                                                        <option value="">Select Account Head</option>
                                                        @foreach($debitAccounts as $key => $y2)
                                                            <option value="{{ $y2->id}}">{{ $y2->code .' ---- '. $y2->name}}</option>
                                                        @endforeach
                                                            ...
                                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                                <div class="table-responsive">
                                                                    <table id="buildyourform" class="table table-bordered">
                                                                        ...
                                                                        <tbody class="addMoreCashSaleDetailRows_1" id="addMoreCashSaleDetailRows_1">
                                                                        <input type="hidden" name="cashSaleDataSection_1[]" class="form-control requiredField" id="cashSaleDataSection_1" value="1" />
                                                                        <tr>
                                                                            <td>
                                                                            ...
                                                                            <td>
                                                                                <input type="number" name="price_1_1" onkeyup="updateAmount(this.id,1,1)" id="price_1_1" placeholder="Price" class="form-control requiredField" />
                                                                            </td>
                                                                            <td>
                                                                                <input type="number" name="qty_1_1" id="qty_1_1" onkeyup="updateAmount(this.id,1,1)" onchange="checkQuantityinStock(this.id,1,1,'category_id_1_1','sub_item_id_1_1')" placeholder="Quantity" class="form-control requiredField" />
                                                                            </td>
                                                                            <td>
                                            ...
                                            <div class="row">
                                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-right">
                                                    <input type="button" class="btn btn-sm btn-primary" onclick="addMoreCashSaleDetailRows('1')" value="Add More Cash Sale's Rows" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="cashSaleSection"></div>
                                <div class="row">
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-right">
                                        {{ Form::submit('Submit', ['class' => 'btn btn-success']) }}
                                        <button type="reset" id="reset" class="btn btn-primary">Clear Form</button>
                                        <input type="button" style="display: none;" class="btn btn-sm btn-primary addMoreCashSale" value="Add More Cash Sale's Section" />
                                    </div>
                                </div>
                                <?php echo Form::close();?>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            var d = 1;
            $('.addMoreCashSale').click(function (e){
                e.preventDefault();
                d++;
                var m = '<?php echo $_GET['m'];?>';
                $.ajax({
                    url: '<?php echo url('/')?>/smfal/makeFormCashSaleVoucher',
                    type: "GET",
                    data: { id:d,m:m},
                    success:function(data) {
                        $('.cashSaleSection').append('<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" id="Demands_'+d+'"><a href="#" onclick="removeDemandsSection('+d+')" class="btn btn-xs btn-danger">Remove</a><div class="lineHeight">&nbsp;</div><div class="panel"><div class="panel-body">'+data+'</div></div></div>');
                    }
                });
            });
        });
        $(".btn-success").click(function(e){
            var cashSales = new Array();
            var val;
            $("input[name='cashSaleSection[]']").each(function(){
                cashSales.push($(this).val());
            });
            var _token = $("input[name='_token']").val();
            for (val of cashSales) {
        ...
        var x = 1;
        function addMoreCashSaleDetailRows(id){
            x++;
            //alert(id+' ---- '+x);
            var m = '<?php echo $_GET['m'];?>';
            $.ajax({
                url: '<?php echo url('/')?>/smfal/addMoreCashSaleDetailRows',
                type: "GET",
                data: { counter:x,id:id,m:m},
                success:function(data) {
                    $('.addMoreCashSaleDetailRows_'+id+'').append(data);
                }
            });
        }

        function removeCashSaleRows(id,counter){
            var elem = document.getElementById('removeCashSaleRows_'+id+'_'+counter+'');
