...
<div class="row">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <input type="checkbox" name="hideShowDetails" id="hideShowDetails" onclick="hideShowDetails(this)">
        <label for="hideShowDetails" class="hideShowDetailsLable">Hide Details</label>
    </div>
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        ...
        <div class="table-responsive">
            ...
            <table class="table table-bordered sf-table-list"
                ...
                <thead>
                    ...
                    <th class="text-center hideShowData">PR Date</th>
                    <th class="text-center">Department Name </th>
                    <th class="text-center">Mode Type </th>
                    <th class="text-center hideShowData">PR  No </th>
                    <th class="text-center">Item Name</th>
                    <th class="text-center hideShowData">PR Item Remarks</th>
                    ...
                    @if ($viewGrn)
                        ...
                        <th class="text-center hideShowData">PI No </th>
                        <th class="text-center hideShowData">PI Date </th>
                    @endif
                ...
                <tbody>
                    ...
                    @if ($viewPO == 0 && !empty($value->purchase_request_no))
                        @continue;
                    @endif
                    <tr>
                        <td>{{++$counter}}</td>
                        <td class="hideShowData">{{($value->demand_date)?date_format(new DateTime($value->demand_date),"d-M-Y") : '-'}}</td>
                        <td>{{$value->sub_department_name}}</td>
                        <td>{{$mode[$value->p_type]}}</td>
                        <td class="hideShowData">{{$value->demand_no}}</td>
                        <td>{{$value->sub_ic}}</td>
                        <td class="hideShowData">{{$value->description}}</td>
                
                        <td>{{$value->remarks?? '-'}}</td>
                    
                        <td>
                            <?php echo $uom =  App\Helpers\CommonHelper::get_uom_by_uom_id($value->uom); ?>
                        </td>
                        ...
                        @if ($viewGrn)
                            ...
                            <td class="hideShowData">{{$value->pv_no?? '-'}}</td>
                            <td class="hideShowData">{{($value->pv_date)?date_format(new DateTime($value->pv_date),"d-M-Y") : '-'}}</td>
                        @endif            
...
<script>
    function hideShowDetails(e) {
        if (document.getElementById('hideShowDetails').checked) {
            $('.hideShowData').toggle();
            $('.hideShowDetailsLable').text("Show Details");
        } else {
            $('.hideShowData').toggle();
            $('.hideShowDetailsLable').text("Hide Details");
        }
    }
