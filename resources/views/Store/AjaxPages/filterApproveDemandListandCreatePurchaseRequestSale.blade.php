    ...
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="panel">
            <div class="panel-body">
                ...
                $approveDemandVoucherListandCreatePurchaseRequestSale = \DB::table("demand_data")
                    ->select("demand_data.demand_no","demand_data.demand_date","demand_data.category_id","demand_data.sub_item_id","demand_data.description","demand_data.id","demand.slip_no","demand.demand_type","demand_data.qty","demand_data.demand_send_type",
                        \DB::raw("(SELECT SUM(purchase_request_qty) as purchase_request_qty FROM purchase_request_data
                        WHERE purchase_request_data.purchase_request_qty != 0 and purchase_request_data.category_id = demand_data.category_id
                        and purchase_request_data.sub_item_id = demand_data.sub_item_id
                        and purchase_request_data.demand_no = demand_data.demand_no
                        ) as purchase_request_qty"))
                    ->join('demand','demand_data.demand_no','=','demand.demand_no')
                    ->whereBetween('demand.demand_date',[$fromDate,$toDate])
                    ->where(['demand.sub_department_id' => $row->id,'demand.status' => '1','demand.demand_type' => '2','demand.demand_status' => '2','demand_data.demand_send_type' => '200000'])
                    ->where('demand_data.purchase_request_status','!=','2')
                    ->get();
                CommonHelper::reconnectMasterDatabase();

                if(count($approveDemandVoucherListandCreatePurchaseRequestSale) == 0){}else{
                ?>
                <?php echo Form::open(array('url' => 'stad/createPurchaseRequestSaleDetailForm?m='.$m.'&&parentCode='.$parentCode.'&&pageType=add#SFR','id'=>'createPurchaseRequestSaleDetailForm_'.$row->id.''));?>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="table-responsive">
                            <table class="table table-bordered table-responsive">
                                ...
                                <?php
                                $counter = 1;
                                foreach ($approveDemandVoucherListandCreatePurchaseRequestSale as $row1){
                                if($row1->purchase_request_qty == $row1->qty){}else{
                                ?>
                                <tr>
                                    ...
                                    <td><?php echo CommonHelper::get_category_name($row1->category_id);?></td>
                                    <td><?php echo CommonHelper::get_item_name($row1->sub_item_id);?></td>
                                    <td><?php echo $row1->description;?></td>
                                    <td class="text-center">
                                        <?php echo $row1->qty;?>
                                    </td>
                                    <td class="text-center">
                                        <?php echo $row1->purchase_request_qty;?>
                                    </td>
                                </tr>
                                <?php
                                }
