                                    ...
                                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                        ...
                                        <select onchange="claculation(1);get_rate()" name="curren" id="curren" class="form-control select2 requiredField">
                                            ...
                                            @foreach(CommonHelper::get_all_currency() as $row)
                                                <option value="{{$row->id.','.$row->rate}}" <?php if($purchase_order->currency_id == $row->id):echo "selected"; endif;?>>{{$row->curreny}}</option>
                                            @endforeach;
                                    ...
                                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                        ...
                                        <select class="form-control requiredField select2" name="company_location_id"
                                            ...
                                            @foreach (ReuseableCode::getUserWiseLocationRightsData() as $company_location)
                                                <option value="{{$company_location['id']}}" {{($purchase_order->company_location_id == $company_location['id'])? 'selected' : ''}}>{{$company_location['location_name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                ...
                                <div class="table-responsive" >
                                    <table class="table table-bordered">
                                        ...
                                        <tr title="1" id="RemoveRows{{$Counter}}" class="AutoNo">
                                            <td>
                                                <input type="hidden" class="form-control requiredField sam_jass" name="sub_ic_des[]" value="{{$Fil->sub_item_id}}"  id="item_<?php echo $Counter?>" placeholder="ITEM">
                                                <select class="form-control select2" name="item_id[]" onchange="itemChange(<?php echo $Counter?>)" id="sub_<?php echo $Counter?>">
                                                    <option value="0">Select Item</option>
                                                    @foreach (CommonHelper::get_all_subitem() as $item)
                                                    <option @if($Fil->sub_item_id == $item->id) selected @endif value="{{ $item->id }}" data-id="{{ $item->id }}" data-uom="{{$item->uomData->uom_name}}">
                                                        {{ $item->sub_ic }}
                                                    </option>
                                                    @endforeach
