...
@foreach ($weighbridges as $key => $weighbridge)
    <tr class="text-center">
        ...
        <td>{{$weighbridge->parent_location ??  '--'}} => {{$weighbridge->location ??  '--'}}</td>
        <td>
            {{-- @if ($view) --}}
            <button
                onclick="showDetailModelOneParamerter('arrival/Viewweighbridge',{{ $weighbridge->id }},'View Weight bridge Details')"
