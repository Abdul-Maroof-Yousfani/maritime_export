...
if(!empty($selectAccountHead)){
    ...
    $accountCondition = ['rvs.sale_receipt_type' => '1'];
}

if($selectVoucherStatus == '0'){
    $voucherStatusTitle = 'All Vouchers';
    $voucherCondition = ['rvs.sale_receipt_type' => '1'];
}else if($selectVoucherStatus == '1'){
    $voucherStatusTitle = 'Pending Vouchers';
    $voucherCondition = ['rvs.sale_receipt_type' => '1','rvs.rv_status' => '1','rvs.status' => '1'];
}else if($selectVoucherStatus == '2'){
    $voucherStatusTitle = 'Approve Vouchers';
    $voucherCondition = ['rvs.sale_receipt_type' => '1','rvs.rv_status' => '2','rvs.status' => '1'];
}else if($selectVoucherStatus == '3'){
    $voucherStatusTitle = 'Deleted Vouchers';
    $voucherCondition = ['rvs.sale_receipt_type' => '1','rvs.status' => '2'];
}
$counter = 1;
$makeTotalAmount = 0;
...
$rvs = DB::table('rvs')
    ->select('rvs.rv_no','rvs.rv_date','rvs.inv_no','rvs.inv_date','rvs.rv_status','rvs.slip_no','rvs.status','rvs.voucherType','rvs.sale_receipt_type','rv_data.acc_id')
...
<tr>
    ...
    <td class="text-center hidden-print"><a onclick="showDetailModelOneParamerter('fdc/viewCashReceiptVoucherDetail','<?php echo $row1->rv_no;?>','View Cash R.V Detail')" class="btn btn-xs btn-success"><span class="glyphicon glyphicon-eye-open"> V</span></a></td>
</tr>
<?php
}
?>
