...
<div id="cost_center<?php echo  $count1 ?>" class="col-lg-4 col-md-4 col-sm-4 col-xs-12 banks removeDemandsRows_dept_1_<?php echo  $count1 ?>">
    <?php
    $data1= CommonHelper::cost_center_allocation_data($data->id,$type);

    ?>
    <div class="row">
        <div id=""  class="col-lg-5 col-md-5 col-sm-5 col-xs-6 banks">
            <input   type="checkbox" name="cost_center_check_box<?php echo $count1 ?>" value="1" id="cost_center_check_box<?php echo $count1 ?>" class="">Allow Null  </label>
        </div>
    ...
    <input type="hidden" name="cost_center_dept_hidden_amount<?php echo  $count1 ?>" id="cost_center_dept_hidden_amount<?php echo  $count1 ?>" value="{{$data->amount}}"/>
    ...
    <table  id="cost_center_table1" class="table table-bordered">
        ...
        @if (!empty($data1))
            @foreach(CommonHelper::cost_center_allocation_data($data->id,$type) as $row2)
                ...
                ?>
            @endforeach
            <?php         $cost_count=  CommonHelper::cost_center_counter($id,$type);

            ?>
            <input type="hidden" id="cost_count" value="{{$cost_count}}" />
        @else
            <?php  ?>
...
<script>
    ...
    var cost_center_allocation=$('#cost_count').val();
    if (typeof(cost_center_allocation) == 'undefined')
    {
        cost_center_allocation=1;
    }

    $(document).ready(function() {
        var main_count='<?php echo $count1; ?>';

        $('.cost_center_department_amount'+main_count).number(true,2);
        $('#cost_center_total_dept'+main_count).number(true,2);
