...
<?php
$data2= CommonHelper::department_allocation_data($data->id,$type);

?>
<div id="dept_allocation<?php echo  $count1 ?>"  <?php if (empty($check_department_allocation)) : ?>  style="display: block" <?php endif; ?>  class="col-lg-4 col-md-4 col-sm-4 col-xs-12 banks removeDemandsRows_dept_1_<?php echo  $count1 ?>">

    <div class="row">
        <div id=""  class="col-lg-5 col-md-5 col-sm-5 col-xs-5 banks">
            <input   type="checkbox" name="dept_check_box{{$count1}}" value="1" id="dept_check_box{{$count1}}" class="">Allow Null</label>
        </div>
        <div id=""   class="col-lg-6 col-md-6 col-sm-6 col-xs-6 banks">
    ...
    <table  id="table1" class="table table-bordered">
        <input type="hidden" name="dept_hidden_amount<?php echo $count1 ?>" id="dept_hidden_amount<?php echo $count1 ?>" value="{{$data->amount}}"/>
        <tbody class="addrows<?php echo $count1; ?>" id="">
        ...
        ?>
        <?php $department_allocation=1; ?>
        @if (!empty($data2))

            @foreach(CommonHelper::department_allocation_data($data->id,$type) as $row1)

                @if($row1->dept_id!='0')
                    <tr>
                        <td style="width: 40%">
                            <select  onchange="open_form(this.id)" style="width: 150px" name="department<?php echo  $count1 ?>[]" id="department_<?php echo $count1 ?>_<?php echo  $department_allocation ?>"
                                     class="form-control select2 requiredField dept1 dept_form">
                                <option value="0">Select Department&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </option>
                                <option  value="add_new"><b>Add New</b>  </option>
                                @foreach($department as $row)
                                    <option @if($row1->dept_id==$row->id)selected @endif  value="{{$row->id}}">{{  ucwords($row->name)}}</option>
                                @endforeach
                            </select>
                        </td>
                        <td>
                            <input value="{{$row1->percent}}"  onkeyup="calculation_dept_item(this.id,'<?php echo $count1 ?>')" placeholder="Percentage" type="any" name="percent<?php echo  $count1 ?>[]" id="percent_<?php echo $count1 ?>_<?php echo  $department_allocation ?>" class="form-control dept1" />
                        </td>
                        <td class="text-right">

                            <input value="{{$row1->amount}}" onblur="add('<?php echo $count1 ?>',this.id)"   onkeyup="calculation_dept_item_amount(this.id,'<?php echo $count1 ?>')" class="department_amount<?php echo  $count1 ?> form-control dept1 number_formate" style="text-align: right" type="text" name="department_amount<?php echo  $count1 ?>[]" id="department_amount_<?php echo $count1 ?>_<?php echo  $department_allocation ?>" />
                        </td><!-->
                    </tr>
                    <?php $total_amount+=$row1->amount; ?>
                @endif
                <?php $department_allocation++; ?>
            @endforeach

            <?php  $dept_count=  CommonHelper::dept_center_counter($id,$type); ?>
            <input type="hidden" id="dept_count" value="{{$dept_count}}" />


        @else
            <tr>
                <td style="width: 40%">
                    <select  onchange="open_form(this.id)" style="width: 190px" name="department<?php echo  $count1 ?>[]" id="department_<?php echo $count1 ?>_1_1"
                             class="form-control select2 requiredField dept1 dept_form">
                        <option value="0">Select Department&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </option>
                        <option  value="add_new"><b>Add New</b>  </option>
                        @foreach($department as $row)
                ...
                <td>
                    <input   onkeyup="calculation_dept_item(this.id,'<?php echo $count1 ?>')" placeholder="Percentage" type="any" name="percent<?php echo  $count1 ?>[]" id="percent_<?php echo $count1 ?>_1" class="form-control dept1" />
                </td>
                <td class="text-right">

                    <input  onblur="add('<?php echo $count1 ?>',this.id)"   onkeyup="calculation_dept_item_amount(this.id,'<?php echo $count1 ?>')" class="department_amount<?php echo $count1 ?> form-control dept1" style="text-align: right" type="text" name="department_amount<?php echo  $count1 ?>[]" id="department_amount_<?php echo $count1 ?>_1" />
                </td><!-->
            </tr>
        @endif
        ...
        <tr>
            ...
            <td   style="text-align: right" id="dept_amount{{$count1}}">0</td>
            <td colspan="2" ><input value="{{$total_amount}}" style="text-align: right;" readonly type="text" class="form-control number_formate" name="total_dept<?php echo $count1 ?>" id="total_dept<?php echo $count1 ?>"></td>
        </tr>
    </table>
</div>
...
<script type="text/javascript">
    ...
    var dept_allocation=$('#dept_count').val();
    if (typeof(dept_allocation) == 'undefined')
    {
        dept_allocation=1;
    }
...
<script type="text/javascript">
    function AddMore(number)
    {
        ...
        dept_allocation++;
        $(wrapper).append('<tr><td style="40%"><select onchange="open_form(this.id)" style="width: 170px;" name="department'+number+'[]" id="department_'+number+'_'+dept_allocation+'" class="form-control select2 dept'+number+' dept_form">' +
                ...
                '<td> <input type="any" onkeyup="calculation_dept_item(this.id,'+number+')" name="percent'+number+'[]" id="percent_'+number+'_'+dept_allocation+'" class="form-control dept'+number+'" /></td>' +
