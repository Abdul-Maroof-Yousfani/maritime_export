...
?>
<input type="hidden" id="srb_amount" value="{{$array[0]}}">
<input type="hidden" id="srb_sales_tax" value="{{$array[1]-$previous_sales_tax}}">
...
<blockquote style="display: none;color: red" id="srb_html" cite="">
...
applicable_rate=parseFloat(applicable_rate);
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#srb_html').html(txt+' '+response[0]+' '+'('+apply_amount+')');
              var  cal_amount=(percent / 100)*apply_amount;

                $('#tax3').fadeIn(500);
                $('#c_amount_1_6').val(cal_amount);

                $('.perc6').html('SRB Withholding '+txt+' '+response[0]);
