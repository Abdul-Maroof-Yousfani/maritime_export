...
<div id="cost_center{{$x}}"  style="display: none;" class="col-lg-4 col-md-4 col-sm-4 col-xs-12 banks">

    <div class="row">
        <div id="dept_allocation{{$x}}"  class="col-lg-5 col-md-5 col-sm-5 col-xs-6 banks">
            <input checked type="checkbox" name="cost_center_check_box{{$x}}" value="1" id="cost_center_check_box{{$x}}" class="">Allow Null  </label>
        </div>


        <div id="dept_allocation{{$x}}"   class="col-lg-6 col-md-6 col-sm-6 col-xs-6 banks">
    ...
    <input type="hidden" name="cost_center_dept_hidden_amount{{$x}}" id="cost_center_dept_hidden_amount{{$x}}"/>

    <table  id="cost_center_table{{$x}}" class="table table-bordered">

        <tbody class="cost_center_addrows{{$x}}" id="">
        <input type="hidden" name="" class="form-control requiredField" id="demandDataSection_{{$x}}" value="1" />
        <tr>
            <td style="width: 40%">
                <select style="width: 180px;" onchange="open_form_cost_center(this.id);check_repeatition(this.id,{{$j}})"  name="cost_center_department{{$x}}[]" id="cost_center_department_{{$x}}_1"
                        class="form-control select2 CostCenter cost_allocation{{$x}}">
            ...
            <td>
                <input onkeyup="cost_center_calculation_dept_item(this.id,'{{$x}}');addclass('cost_center_department_amount_{{$x}}_1','{{$x}}_1')" placeholder="Percentage" type="any" name="cost_center_percent{{$x}}[]" id="cost_center_percent_{{$x}}_1" class="form-control" />
            </td>
            <td class="text-right">

                <input onblur="add_cost_center('{{$x}}',this.id)"  onkeyup="cost_center_calculation_dept_item_amount(this.id,'{{$x}}');addclass(this.id,'{{$x}}_1')" class="cost_center_department_amount{{$x}} form-control" style="text-align: right" type="text" name="cost_center_department_amount{{$x}}[]" id="cost_center_department_amount_{{$x}}_1" class="form-control" />
            </td><!-->
        </tr>
        ...
        <tr>
            ...
            <td   style="text-align: right" id="cost_center_dept_amount{{$x}}">0</td>
            <td colspan="1"><input style="text-align: right;font-size: 12px" readonly type="text" class="form-control" name="cost_center_total_dept{{$x}}" id="cost_center_total_dept{{$x}}"></td>
        </tr>
    </table>
</div>
<!--
...
<script>
    ...
    function cost_center_AddMore(number)
    {
        ...
        cost_center_allocation++;
        $(wrapper).append('<tr><td style="width: 40%"><select onchange="open_form_cost_center(this.id);check_repeatition(this.id,'+number+')" style="width: 170px;" name="cost_center_department'+number+'[]" id="cost_center_department_'+number+'_'+cost_center_allocation+'" class="form-control select2 CostCenter cost_allocation'+number+'">' +
                '<option value="">Select</option>   <option  value="add_new"><b>Add New</b>  </option>@foreach(CommonHelper::get_all_cost_center() as $row)<option value="<?php echo $row->id ?>"><?php echo ucwords($row->name)?></option>@endforeach</select></td>' +
                '<td> <input type="any" onkeyup="cost_center_calculation_dept_item(this.id,'+number+');addclass(this.id,`'+number+'_'+cost_center_allocation+'`)" name="cost_center_percent'+number+'[]" id="cost_center_percent_'+number+'_'+cost_center_allocation+'" class="form-control" /></td>' +
                '<td class="text-right"><input onblur="add_cost_center('+number+',this.id)" onkeyup="cost_center_calculation_dept_item_amount(this.id,'+number+');addclass(this.id,`'+number+'_'+cost_center_allocation+'`)" class="cost_center_department_amount'+number+' form-control" style="text-align: right"  type="text" name="cost_center_department_amount'+number+'[]" id="cost_center_department_amount_'+number+'_'+cost_center_allocation+'" class="form-control" /></td></tr>');
    ...
    function cost_center_colour_change(number1,total_dept_amountt,actual_amount)
    {
        ...
        if (amount==1)
        {
            ...
            $("#cost_center_total_dept"+number1).delay(3000).css("background-color","#e9ea9d");
        }
        else
        {
            $("#cost_center_total_dept"+number1).delay(3000).css("background-color","#e2a0a0");
        }
    }
    ...
    function cost_center_heading_change(number)
    {
        ...
        var dept_total_amount= parseFloat($('#cost_center_total_dept'+number).val());
    ...
    function cost_center_allocation_amount_display(id)
    {
        ...
        var amount= $('#d_amount_1_'+id+'').val();
        if (amount==0)
        {
            amount= $('#c_amount_1_'+id+'').val();
        }
        if (amount>0)
        {
    ...
    function addclass(id,number)
    {
        ...
        if (value>0)
        {
        //    $("#department_"+number).addClass("requiredField");
            $("#cost_center_department_"+number).addClass("requiredField");
        }
        else
        {
        //    $("#department_"+number).removeClass("requiredField");
           $("#cost_center_department_"+number).removeClass("requiredField");
        }

    }

    function addclasstwo(id,number)
    {
    ...
    function check_repeatition(id,clas) {

        var fisrt_id = clas;
        clas = 'cost_allocation' + clas;
        ...
        $('.' + clas).each(function (i, obj) {
            ...
            if (id != obj.id) {
                ...
                if ($('#' + id).val() == value && $('#' + id).val()!='') {
                    ...
                    alert('IT"S ALREADY SELECTED ON LINE NO' + ' ' + counter);
