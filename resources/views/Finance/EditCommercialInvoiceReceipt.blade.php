...
// Get existing receipt data for pre-filling
$existingReceiptData = [];
foreach($rvsData as $data) {
    $existingReceiptData[$data->commercial_invoice_id] = $data;
}

// Get existing account ID from new_rv_data
$existingAccId = DB::Connection('mysql2')->table('new_rv_data')
    ->where('master_id', $rvs->id)
    ->where('debit_credit', 1)
    ->where('status', 1)
    ->where('acc_id', '!=', 0)
    ->first();
$existingAccId = $existingAccId ? $existingAccId->acc_id : '';

?>

 <?php echo Form::open(array('url' => 'fad/updateCommercialInvoiceReceipt?m='.$_GET['m'].'','id'=>'editCommercialInvoiceReceipt','class'=>'stop'));?>
<input type="hidden" name="id" value="{{$rvs->id}}">
<div class="well_N">
    <div class="receipt-voucher-title-wrapper">
        <h2>Edit Receipt Voucher (Commercial Invoice)</h2>
        ...
        <div class="well">
            <div class="row">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                     ...
                     <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                      <label for="email">Voucher Date</label>
                       <input type="date" value="{{$rvs->rv_date ? date('Y-m-d', strtotime($rvs->rv_date)) : date('Y-m-d')}}" class="form-control" id="v_date" name="v_date">
                       </div>

                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                        <label for="pwd">Payment Mode</label>
                        <select id="pay_mode" name="pay_mode" onchange="hide_unhide()" class="form-control">
                            <option value="1,1" {{$rvs->pay_mode == 1 ? 'selected' : ''}}>Cheque</option>
                            <option value="2,2" {{$rvs->pay_mode == 2 ? 'selected' : ''}}>Cash </option>
                        </select>
                    </div>

                    <div  class="col-lg-3 col-md-3 col-sm-3 col-xs-12 hidee" style="display:{{$rvs->pay_mode == 1 ? 'block' : 'none'}};">
                        <label for="pwd"> Banks </label>
                        <?php $bank=DB::Connection('mysql2')->table('bank_detail')->get(); ?>
                        <select name="bank" class="form-control">
                            @foreach($bank as $row)
                            <option value="{{$row->id}}" {{$rvs->bank == $row->id ? 'selected' : ''}}>{{$row->bank_name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 hidee" style="display:{{$rvs->pay_mode == 1 ? 'block' : 'none'}};">
                        ...
                        <input type="text" class="form-control" id="cheque" name="cheque" value="{{$rvs->cheque_no}}">
                    </div>

                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 hidee" style="display:{{$rvs->pay_mode == 1 ? 'block' : 'none'}};">
                        <label for="pwd">Cheque Date:</label>
                        <input value="{{$rvs->cheque_date ? date('Y-m-d', strtotime($rvs->cheque_date)) : date('Y-m-d')}}" class="form-control" name="cheque_date" type="date" >
                    </div>

                    <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12" id="dr_account_div" style="display:{{$rvs->pay_mode == 2 ? 'block' : 'none'}};">
                        ...
                        <select name="acc_id" id="acc_id" class="form-control select2">
                            ...
                            @foreach(CommonHelper::get_all_account() as $row)
                                <option value="{{$row->id}}" {{$existingAccId == $row->id ? 'selected' : ''}}>{{$row->name}}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                    ...
                    <textarea name="desc" class="form-control" rows="3" id="comment">{{$rvs->description}}</textarea>
                </div>
            </div>
        ...
        <input type="hidden" name="ref_bill_no" value="{{$rvs->ref_bill_no}}" />
    <div>&nbsp;</div>
    ...
    <table id="" class="table table-bordered" >
        ...
        <tbody>
            ...
            @foreach($val as $row)
              ...
              // Get received payment for this commercial invoice (excluding current receipt)
              CommonHelper::companyDatabaseConnection($_GET['m']);
              $allReceived = SalesHelper::get_received_payment_commercial_invoice($row);
              // Subtract current receipt amount to get previous received
              $currentReceiptAmount = isset($existingReceiptData[$row]) ? $existingReceiptData[$row]->received_amount : 0;
              $received_amount = $allReceived - $currentReceiptAmount;
              ...
              // Get existing received amount from receipt data
              $existingReceivedAmount = isset($existingReceiptData[$row]) ? $existingReceiptData[$row]->received_amount : ($invoice_amount - $received_amount);

              $invoice_no[]=$commercialInvoice->invoice_no;
              $totalNetAmount += $existingReceivedAmount;
              ?>
            ...
            <td><input class="form-control receive_amount" onkeyup="calc('{{$invoice_amount}}','{{$received_amount}}','{{$counter}}','{{$return_amount}}',1)" 
             onblur="calc('{{$invoice_amount}}','{{$received_amount}}','{{$counter}}','{{$return_amount}}',1)" type="text" name="receive_amount[]" id="receive_amount{{$counter}}"
                      value="{{number_format($existingReceivedAmount, 2, '.', '')}}"
                        ></td>
            ...
            <td><input class="form-control net_amount comma_seprated" type="text" readonly value="{{number_format($existingReceivedAmount, 2, '.', '')}}" name="net_amount[]" id="net_amount{{$counter}}"></td>
        ...
        <td  id=""><input readonly type="text" id="net_total" class="form-control comma_seprated" value="{{number_format($totalNetAmount, 2, '.', '')}}"/> </td>
        </tr>
    ...
    <button   type="submit" class="btn btn-success" onclick="SetValue(0)">Update</button>
    <button type="submit" id="BtnSaveAndPrint" class="btn btn-info BtnSaveAndPrint" onclick="SetValue(1)">Update & Print</button>
    </div>
</div>
{{Form::close()}}
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#net_total').val(amount.toFixed(2));
        });
