    ...
    <div class="row">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="well">
                ...
                <div class="row">
                    <?php echo Form::open(array('url' => 'fad/updateJournalVoucherDetail?m='.$m.'','id'=>'updateJournalVoucherDetail'));?>
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="pageType" value="viewlist">
                    <input type="hidden" name="parentCode" value="33">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="panel">
                            <div class="panel-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                                                ...
                                                <input  readonly type="text" class="form-control requiredField" placeholder="Slip No"
                                                        name="voucher_no" id="voucher_no" value="{{strtoupper($jvs->jv_no)}}" />
                                            </div>

                                            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                                                <label class="sf-label">JV Date.</label>
                                                <span style="font-size:17px !important; color:#F5F5F5 !important;"><strong>*</strong></span>
                                                <input onblur="change_day()" onchange="change_day()" type="date" class="form-control requiredField" max="<?php echo date('Y-m-d') ?>" name="jv_date_1" id="pv_date_1" value="<?php echo $jvs->jv_date ?>" />
                                            </div>
                                            ...
                                            <div class="col-lg-2 col-md-2 col-sm-2  col-xs-12">
                                                ...
                                                <span style="font-size:17px !important; color:#F5F5F5 !important;"><strong>*</strong></span>
                                                <input autofocus  type="text" class="form-control requiredField" placeholder="Slip No" name="slip_no_1" id="slip_no_1" value="<?php echo $jvs->slip_no?>" />
                                            </div>

                                            <div class="col-lg-2 col-md-2 col-sm-2  col-xs-12">
                                                <label class="sf-label">Bill Date</label>
                                                <span style="font-size:17px !important; color:#F5F5F5 !important;"><strong>*</strong></span>
                                                <input autofocus  type="date" class="form-control requiredField" name="bill_date" id="bill_date" value="<?php echo $jvs->bill_date?>" />
                                            </div>

                                            <div class="col-lg-2 col-md-2 col-sm-2  col-xs-12">
                                                <label class="sf-label">Due Date</label>
                                                <span style="font-size:17px !important; color:#F5F5F5 !important;"><strong>*</strong></span>
                                                <input autofocus  type="date" class="form-control requiredField" name="due_date" id="due_date" value="<?php echo $jvs->due_date?>" />
                                            </div>
                                ...
                                <div class="checkbox">
                                    <label><input onclick="show()" name="items" @if($jvs->with_items==1) checked @endif type="checkbox" id="items" value="1">With Items</label>
                                </div>
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="table-responsive">
                                            <table id="buildyourform" class="table table-bordered  sf-table-th sf-table-form-padding">
                                                ...
                                                foreach ($jv_data as $jv_dataFiltr):
                                                ?>
                                                <input type="hidden" name="jvsDataSection_1[]" class="form-control requiredField" id="jvsDataSection_1" value="<?php echo $counter?>" />
                                                <tr>
                                                    <td>
                                                        <select   onchange="get_current_amount(this.id);check_supplier(this.id)" style="width: 100%" class="form-control requiredField select2 accounts" name="account_id_1_<?php echo $counter?>" id="account_id_1_<?php echo $counter?>">
                                                            ...
                                                            @foreach(CommonHelper::get_accounts_for_jvs() as $key => $y)
                                                                <option <?php if($y->id == $jv_dataFiltr['acc_id']){echo "selected";}?> value="{{ $y->id.'~'.$y->supp_id.'~'.$y->supplier_id}}">{{$y->name}}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    ...
                                                    <td class="hidee">
                                                        <select   onchange="get_detail_purchase_voucher(this.id)" style="width: 200px;" name="sub_item_id_1_<?php echo $counter ?>" id="sub_item_id_1_<?php echo $counter ?>" class="form-control select2">
                                                            ...
                                                            @foreach(CommonHelper::get_all_subitem() as $row)
                                                                <option <?php if($row->id == $jv_dataFiltr['item_id'] ){echo "selected";} ?> value="{{ $row->id }}">{{ ucwords($row->sub_ic)  }}</option>
                                                            @endforeach
                                                        </select>
                                                    </td>
                                                    <?php
                                                    $uom_name='';
                                                    if (!empty($jv_dataFiltr['item_id'])):
                                                        $sub_item_data=CommonHelper::get_subitem_detail($jv_dataFiltr['item_id']);
                                                    ...
                                                    <td class="hidee">
                                                        <input onkeyup="calculation_amount(this.id);dept_cost_amount('d_amount_1_<?php echo $counter ?>','<?php echo $counter ?>')"  type="number" step="0.01" name="qty_1_<?php echo $counter ?>" id="qty_1_<?php echo $counter ?>" class="form-control qty" value="{{$jv_dataFiltr['qty']}}" />
                                                    </td>


                                                    <td class="hidee">
                                                        <input  onkeyup="calculation_amount(this.id);dept_cost_amount('d_amount_1_<?php echo $counter ?>','<?php echo $counter ?>')" type="text" step="0.01" name="rate_1_<?php echo $counter ?>" id="rate_1_<?php echo $counter ?>" class="form-control rate" value="{{$jv_dataFiltr['rate']}}" />
                                                    </td>
                                                    ...
                                                    <td>
                                                        <input @if($jv_dataFiltr['debit_credit']==1) <?php $total_debit+=$jv_dataFiltr['amount'] ?> value="{{$jv_dataFiltr['amount']}}" @else readonly @endif    onfocus="mainDisable('c_amount_1_<?php echo $counter ?>','d_amount_1_<?php echo $counter ?>');"
                                                    ...
                                                    <td>
                                                        <input   @if($jv_dataFiltr['debit_credit']==0) <?php $total_credit+=$jv_dataFiltr['amount'] ?> value="{{$jv_dataFiltr['amount']}}"   @else  readonly @endif  onfocus="mainDisable('d_amount_1_<?php echo $counter ?>','c_amount_1_<?php echo $counter ?>');" placeholder="Credit" class="form-control requiredField c_amount_1" maxlength="15" min="0" type="text" name="c_amount_1_<?php echo $counter ?>" id="c_amount_1_<?php echo $counter ?>" onkeyup="sum('1');calculation(this.id,'0');dept_cost_amount(this.id,'<?php echo $counter ?>')" value="" required="required"/>
                                                    </td>
                                                    <?php
                                                    $check_department_allocation= CommonHelper::department_allocation_data($jv_dataFiltr['id'],$type);
                                                    $check_costing_allocation= CommonHelper::cost_center_allocation_data($jv_dataFiltr['id'],$type);
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                    <div class="row">
                                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                            ...
                                            <textarea  name="description_1" id="description_1" style="resize:none;" class="form-control requiredField"><?php echo $jvs->description?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
