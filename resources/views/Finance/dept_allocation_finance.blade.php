...
<script>
        ...
        <div id="dept_allocation{{$x}}"  style="display: none"  class="col-lg-4 col-md-4 col-sm-4 col-xs-12 banks">

    <div class="row">
        <div id="dept_allocation{{$x}}"  class="col-lg-5 col-md-5 col-sm-5 col-xs-5 banks">
            <input checked type="checkbox" name="dept_check_box{{$x}}" value="1" id="dept_check_box{{$x}}" class="">Allow Null</label>
        </div>
    ...
    <table  id="table1" class="table table-bordered">
        <input type="hidden" name="dept_hidden_amount{{$x}}" id="dept_hidden_amount{{$x}}"/>
        <tbody class="addrows{{$x}}" id="">
        <input type="hidden" name="" class="form-control requiredField" id="" value="1" />
        <tr>
            <td style="width: 40%">
                <select  onchange="open_form(this.id)" style="width: 190px" name="department{{$x}}[]" id="department_{{$x}}_1"
                         class="form-control select2 dept1 dept_allocation{{$x}} dept_form">
            ...
            <td>
                <input  onkeyup="calculation_dept_item(this.id,'{{$x}}')" placeholder="Percentage" type="any" name="percent{{$x}}[]" id="percent_{{$x}}_1" class="form-control dept{{$x}}" />
            </td>
            <td class="text-right">

                <input onblur="add('{{$x}}',this.id)"   onkeyup="calculation_dept_item_amount(this.id,'{{$x}}')" class="department_amount{{$x}} form-control dept{{$x}}" style="text-align: right" type="text" name="department_amount{{$x}}[]" id="department_amount_{{$x}}_1" />
            </td><!-->
        </tr>
        ...
        <tr>
            ...
            <td   style="text-align: right" id="dept_amount{{$x}}">0</td>
            <td colspan="2" ><input style="text-align: right;" readonly type="text" class="form-control" name="total_dept{{$x}}" id="total_dept{{$x}}"></td>
        </tr>
    </table>
</div>
...
<script type="text/javascript">
    function AddMore(number)
    {
        ...
        $(wrapper).append('<tr><td style="40%"><select onchange="open_form(this.id)" style="width: 170px;" name="department'+number+'[]" id="department_'+number+'_'+dept_allocation+'" class="form-control select2 dept'+number+' dept_form">' +
                '<option value="">Select</option> <option  value="add_new"><b>Add New</b> </option>@foreach($department as $row)<option value="<?php echo $row->id ?>"><?php echo  ucwords($row->name)?></option>@endforeach</select></td>' +
                '<td> <input type="any" onkeyup="calculation_dept_item(this.id,'+number+');addclass(this.id,`'+number+'_'+dept_allocation+'`)" name="percent'+number+'[]" id="percent_'+number+'_'+dept_allocation+'" class="form-control dept'+number+'" /></td>' +
                '<td class="text-right"><input onblur="add('+number+',this.id)" onkeyup="calculation_dept_item_amount(this.id,'+number+');addclass(this.id,`'+number+'_'+dept_allocation+'`)" class="department_amount'+number+' form-control  dept'+number+'" style="text-align: right"  type="text" name="department_amount'+number+'[]" id="department_amount_'+number+'_'+dept_allocation+'" /></td></tr>');
    ...
    function colour_change(number1,total_dept_amountt,actual_amount)
    {
        ...
        if (amount==1)
        {
            ...
            $("#total_dept"+number1).delay(3000).css("background-color","#e9ea9d");
        }
        else
        {
            $("#total_dept"+number1).delay(3000).css("background-color","#e2a0a0");
        }
    }
