...
?>
    ...
    <?php echo Form::open(array('url' => 'fad/addEmployeeTaxDetail?m='.$m.'','id'=>'EmployeeTax'));?>
        ...
        <div class="panel-body">
            <div class="row">
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                    ...
                    <select name="tax_id" class="form-control requiredField" id="tax_id" required>
                        <option value="">Select</option>
                        @foreach($taxesList as $value1)
                            <option @if($employeeTax->tax_id == $value1->id) selected @endif value="{{ $value1->id }}">{{ $value1->tax_name }}&nbsp;&nbsp;&nbsp;||&nbsp;&nbsp;&nbsp;
                                ({{ $value1->salary_range_from}}--{{ $value1->salary_range_from}})&nbsp;&nbsp;&nbsp;||&nbsp;&nbsp;&nbsp;
                                %{{ $value1->tax_percent }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                    ...
                    <select class="form-control requiredField" name="department_id" id="department_id" required>
                        ...
                        @foreach($departmentList as $key => $y)
                            <option @if($employeeTax->emp_department_id == $y->id) selected @endif value="{{ $y->id }}">{{ $y->department_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                    ...
                    <select class="form-control requiredField" name="employee_id" id="employee_id" required>
                        ...
                        @foreach($employeeList as $value)
                            <option  @if($employeeTax->id == $value->id) selected @endif value="{{ $value->id }}" >{{ $value->emp_name}}</option>
                        @endforeach
                    </select>
