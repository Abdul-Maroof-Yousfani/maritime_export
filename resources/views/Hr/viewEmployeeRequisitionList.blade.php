				...
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
							<span class="subHeadingLabelClass">View Employee Requisition List</span>
						</div>
							...
							<div class="col-lg-12 col-md-12 col-sm-12col-xs-12">
								<div class="table-responsive">
									<table class="table table-bordered sf-table-list" id="HiringRequestList">
										...
										<tbody>
											<?php $counter = 1;?>
											@foreach($EmployeeRequisition as $key => $y)
												<tr>
													<td class="text-center"><?php echo $counter++;?></td>
													<td class="text-center"><?php echo $y['job_title'];?></td>
													...
													<td class="text-center"><?php echo HrHelper::getApprovalStatusLabel($y['approval_status']);?></td>
													...
													<div class="dropdown">
														...
														<span class="caret"></span></button>
															<ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
																@if(in_array('view',$crud_permission))
																<li role="presentation">
																	<a type="button" onclick="showDetailModelTwoParamerterJson('hdc/viewEmployeeRequisitionDetail','<?php echo $y['id'];?>','View Employee Requisition Detail','<?php echo $m; ?>')" class="edit-modal btn">
																		View
																	</a>
																</li>
																@endif
															@if ($y['approval_status'] != 2 )
																	@if(in_array('approve',$crud_permission))
																		<li role="presentation">
																			<a class="edit-modal btn" onclick="approveAndRejectEmployeeRequisition('<?php echo $m ?>','<?php echo $y['id'];?>','2')">
																				Approve
																			</a>
																		</li>
																	@endif
																...
																@if ($y['approval_status'] != 3)
																		@if(in_array('reject',$crud_permission))
																			<li role="presentation">
																				<a class="edit-modal btn" onclick="approveAndRejectEmployeeRequisition('<?php echo $m ?>','<?php echo $y['id'];?>','3')">
																					Reject
																				</a>
																			</li>
																		@endif
																@endif
																@if(in_array('edit',$crud_permission))
																	<li role="presentation">
																		<a class="edit-modal btn" onclick="showMasterTableEditModel('hr/editEmployeeRequisitionForm','<?php echo $y['id'] ?>','Request Hiring Edit Detail Form','<?php echo $m?>')">
																			Edit
																		</a>
																	</li>
																@endif
																...
																@if ($y['status'] == 2)
																	@if(in_array('repost',$crud_permission))
																		<li role="presentation">
																			<a class="edit-modal btn" onclick="repostOneTableRecords('<?php echo $m ?>','<?php echo $y['id'] ?>','employee_requisition','status')" type="button">
																				Repost
																			</a>
																		</li>
																	@endif
																@else
																		@if(in_array('delete',$crud_permission))
																			<li role="presentation">
																				<a class="delete-modal btn" onclick="deleteRowCompanyHRRecords('<?php echo $m ?>','<?php echo $y['id'] ?>','employee_requisition')">
																					Delete
																				</a>
																			</li>
																		@endif
