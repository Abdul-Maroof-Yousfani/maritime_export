                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered sf-table-list" id="LeaveTypeList">
                                    ...
                                    @foreach($employee->get() as $value)
                                        <?php $url = url('/').Storage::url($value->cnic_path); ?>
                                        <tr class="remove_row_<?=$value->id?>">
                                            ...
                                            <td class="text-center"><a target="_blank" href="<?= Storage::url($value->cnic_path)?>"><?=$value->cnic_name?></a></td>
                                            <td class="text-center">.<?=$value->cnic_type?></td>
                                            <td class="text-center"><a target="_blank" href="<?=$url?>">Download</a></td>
                                            <td class="text-center">
                                                <button data-toggle="tooltip" data-placement="right" title="Delete" onclick="deleteEmployeeCnicCopy('<?= Input::get('m') ?>','<?=$value->id?>')" class="btn btn-xs btn-danger" type="button">
...
<script>
    function deleteEmployeeCnicCopy(company_id,id) {

        var data = {'companyId':company_id,'id':id,'tableName':'employee'};
        var url= '<?php echo url('/')?>/cdOne/deleteEmployeeCnicCopy';
