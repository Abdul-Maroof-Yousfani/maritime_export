                    ...
                    <div class="well">
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                <span class="subHeadingLabelClass">View Arrears List</span>
                            </div>
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12col-xs-12">
                                        <div class="table-responsive">
                                            <table class="table table-bordered sf-table-list table-hover" id="DiseaseList">
                                                ...
                                                <tbody>
                                                <?php $counter = 1;?>
                                                @foreach($arrears->get() as $key => $y)
                                                    <tr>
                                                        <td class="text-center">{{ $counter++ }}</td>
                                                        <td class="text-center">{{ $y->emp_code }}</td>
                                                        <td>{{ HrHelper::getCompanyTableValueByIdAndColumn(Input::get('m'),'employee','emp_name', $y->emp_code, 'emp_code') }}</td>
                                                        <td>{{ HrHelper::getMasterTableValueById(Input::get('m'),'department','department_name', $y->emp_department_id) }}</td>
                                                        <td class="text-right">{{ number_format($y->arrears_amount,0) }}</td>
                                                        <td class="text-center">{{ $y->month }}</td>
                                                            ...
                                                            <div class="dropdown">
                                                                ...
                                                                <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                                                                    @if(in_array('delete', $operation_rights))
                                                                        @if($y->status == 1)
                                                                            <li role="presentation">
                                                                                <a class="delete-modal btn" onclick="deleteRowCompanyRecords('{{ $m }}','{{ $y->id }}','arrears')">
                                                                                    Delete
                                                                                </a>
                                                                            </li>
                                                                        @endif
                                                                    ...
                                                                    @if(in_array('repost', $operation_rights))
                                                                        @if($y->status == 2)
                                                                            <li role="presentation">
                                                                                <a class="delete-modal btn" onclick="repostOneTableRecords('{{ $m }}','{{ $y->id }}','arrears')">
                                                                                    Repost
                                                                                </a>
                                                                            </li>
                                                                        @endif
