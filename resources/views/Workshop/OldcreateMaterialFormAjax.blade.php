...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    <div class="row">
        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
            ...
            <input type="text" readonly class="form-control requiredField" name="voucher_no" value="{{ CommonHelper::getWMIVoucherNumber() }}">
                    ...
                    <tbody>
                        ...
                        @foreach ($grnData as $key => $row)
                            ...
                            <tr class="text-center">
                                ...
                                <td id="grnQty{{$key}}">{{$row->qty}}</td>
                                <td>
                                    <button type="button"
                                        onclick="add_more('{{ $row->item_id }}','{{ $item_name }}','{{ $row->id }}')"
                                        class="btn btn-primary add-more">Add More</button>
                                </td>
                            </tr>
                    <tbody class="{{ $row->item_id }}">
...
<script type="text/javascript">
    ...
    function add_more(append, name, grn_data_id) {
        count++;
        var newRowHtml = '<tr class="text-center" id="departmentRow'+ count +'">' +
            '<td>' + name + '</td>' +
           ...
           ' </select>'+
            '</td>' +
            '<td class="uom"></td>' +
            '<td><input name="item_qty[]" id="item_qty' + count +
            '" type="number" step="any" class="form-control requiredField"></td>' +
            '<td>' +
            '<button class="btn btn-danger btn-xs" onClick="removeRow('+ count +')">Remove</button>'
            '</td>' +
            '</tr>';
