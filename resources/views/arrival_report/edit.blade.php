            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <div class="well_N">
                    <div class="dp_sdw">
                        ...
                        <form method="post" action="{{ route('arrival_report.update', $arrival->id) }}" id="addSubItemDetail" enctype="multipart/form-data">
                                    ...
                                    <div class="panel-body">
                                        <div class="row">
                                            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                                ...
                                                <select disabled class="form-control requiredField select2"
                                                    ...
                                                    @foreach ($company_locations as $company_location)
                                                        <option value="{{ $company_location['id'] }}" {{$arrival->company_location_id == $company_location['id'] ? 'selected' : ''}}>
                                            ...
                                            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                                ...
                                                <span class="rflabelsteric"><strong>*</strong></span>
                                                <input type="text" disabled max="<?php echo date('Y-m-d'); ?>" name="arival_date" id="date"
                                        ...
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                ...
                                                <textarea disabled class="form-control" name="arrival_remarks" id="arrival_remarks" cols="5" rows="5">{{$arrival->arrival_remarks ?? ''}}</textarea>
                                            </div>
                                        </div>
                                        <div class="lineHeight">&nbsp;</div>
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                <div class="table-responsive">
                                                    <table class="table table-bordered">
                                                        ...
                                                        <tbody id="AppnedHtml">
                                                            @foreach ($arrival->arrival_report ?? [] as $key => $detail)
                                                                <tr class="cnt" id="removeSection{{$key}}">
                                                                    <td>
                                                                        <select style="width:200px!important;" onchange="get_item_name({{$key}});get_pr_or_po(this.value,{{$key}})" name="item[]" id="item_id_{{$key}}" class="form-control requiredField select2 item_id">
                                                                            ...
                                                                            <option value="{{CommonHelper::get_item_name($detail->item_id)}}%{{$detail->uom}}%0%{{$detail->item_id}}" selected>{{CommonHelper::get_item_name($detail->item_id)}}</option>
                                                                    ...
                                                                    <td>
                                                                        <input style="width:200px!important;" type="text" class="form-control requiredField "  name="igp_no[]" id="igp_no_{{$key}}"  value="{{$detail->igp_no}}">
                                                                    </td>
                                                                    <td>
                                                                        <input style="width:200px!important;" type="text" class="form-control requiredField "  name="dc_no[]" id="dc_no_{{$key}}"  value="{{$detail->dc_no}}">
                                                                    </td>
                                                                    <td>
                                                                        <select style="width:200px!important;" name="pr_no[]" id="pr_no_{{$key}}"
                                                                            onchange="get_selected_form({{$key}})"
                                                                            class="form-control requiredField select2">
                                                                            @if ($detail->pr_po_no == 0)
                                                                              <option value="0" selected>N/A</option>
                                                                            @else
                                                                            <option value="{{$detail->pr_po_no}}" selected>{{$detail->pr_po_no}}</option>
                                                                            @endif
                                                                        </select>
                                                                    </td>
                                                                    <td>
                                                                        <select style="width:200px!important;"  class="form-control requiredField select2"
                                                                            name="department_id[]" id="department_id_{{$key}}">
                                                                            <option value="">Select Department</option>
                                                                            @foreach ($subDepartmentList as $key3 => $y2)
                                                                                {{-- <optgroup label="{{ $y->department_name }}"
                                                                                    ...
                                                                                    @foreach ($subdepartments as $key2 => $y2) --}}
                                                                                        <option value="{{ $y2->id }}" {{$detail->department_id ==  $y2->id ? 'selected' : ''}}>
                                                                    ...
                                                                    <td>
                                                                        <input style="width:200px!important;" type="text" class="form-control requiredField "  name="qty_requested[]" id="qty_requested_{{$key}}"  value="{{$detail->qty_requested}}">
                                                                    </td>
                                                                    <td>
                                                                        <input style="width:200px!important;" type="text" class="form-control requiredField " name="qty_approved[]" id="qty_approved_{{$key}}" value="{{$detail->qty_approved}}">
                                                                    </td>
                                                                    <td>
                                                                        <select style="width:200px!important;" name="vendor_id[]" id="vendor_id_{{$key}}"
                                                                            class="form-control requiredField select2">
                                                                            <option value="">Select Supplier</option>
                                                                                @foreach ($suppliers as $key2 => $supplier)
                                                                                    <option value="{{ $supplier->id }}" {{$detail->vendor_id ==  $supplier->id ? 'selected' : ''}}>
                                                                                        {{ $supplier->name }}</option>
                                                                                @endforeach  
                                                                        </select>
                                                                    </td>
                                                                
                                                                    <td><span style="cursor:pointer" {{$key == 0 ? 'disabled' : ''}} class="btn btn-danger btn-xs" onClick="removeSection({{$key}})">Remove</span>
                                                                    </td>
                                                                </tr>
                                                            @endforeach
