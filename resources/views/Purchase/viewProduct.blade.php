                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12col-xs-12">
                                    <div class="table-responsive">
                                        <table class="table table-bordered sf-table-list" id="EmpExitInterviewList">
                                            ...
                                            $paramOne = "sdc/editProductForm?m=".$m;
                                            ?>
                                            ...
                                            @foreach($product as $row)
                                                ...
                                                <tr title="{{$row->product_id}}" id="{{$row->product_id}}">
                                                    <td class="text-center">{{$counter++}}</td>
                                                    <td class="text-center">{{ $row->p_name }}</td>
                                                    <td class="text-center"><?php echo CommonHelper::get_account_name_by_code($row->acc_head) ?></td>
                                                    <td class="text-center"><?php
                                                        $type = CommonHelper::get_all_product_type_byID($row->type_id);
                                                        echo $type['type'];
                                                        ?>
                                                    </td>
                                                    <td class="text-center">
                                                        <button onclick="showDetailModelOneParamerter('<?= $paramOne ?>','<?= $row->product_id?>','Edit Product Form')" type="button" class="btn btn-success btn-sm">EDIT</button>
                                                        <button class="btn btn-sm btn-danger" onclick="delete_record('<?php echo $row->product_id; ?>')"> DELETE </button>
                                                    </td>
                                                </tr>
