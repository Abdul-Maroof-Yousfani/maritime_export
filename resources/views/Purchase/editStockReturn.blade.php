...
use App\Helpers\CommonHelper;
       ...
       // print_r($stock_return);
       // print_r($stock_return_data);
$issuance_type = $stock_return->issuance_type;
$is_no = $stock_return->issuance_no;
$issuance_date = $stock_return->issuance_date;
$region = $stock_return->region;
$joborder = $stock_return->joborder;
$description = $stock_return->description;
...
@section('content')
    ...
    <script !src="">
        $(document).ready(function(){
            $('#ShowHideAll').css('display','none');
            IssuanceType('<?= $issuance_type; ?>');
        });
        function IssuanceType(Type)
        {
            if(Type==1)
            {
                $('#ShowHideAll').css('display','block');
                $('.DeliveryChallan').css('display','none');
                $('#IssueWithJobOrder1').css('display','block');
                $('#IssueWithJobOrder2').css('display','block');
                $('#region').addClass('requiredField');
                $('#joborder').addClass('requiredField');
                $('#region_from').removeClass('requiredField');
                $('#region_to').removeClass('requiredField');
                $('input[name="IssuanceType"]').val(Type);
                $("#IssType").val(Type);
            }
            else if(Type==3)
            {
                $('#ShowHideAll').css('display','block');
                $('.DeliveryChallan').css('display','none');
                $('#IssueWithJobOrder2').css('display','none');
                $('#IssueWithJobOrder1').css('display','block');
                $('#region').addClass('requiredField');
                $('#joborder').removeClass('requiredField');
                $('#region_from').removeClass('requiredField');
                $('#region_to').removeClass('requiredField');
                $('input[name="IssuanceType"]').val(Type);
                $("#IssType").val(Type);
    ...
    <div class="row">
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="well">
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <span class="subHeadingLabelClass">Stock Return</span>
                    </div>
                </div>
                <div class="row">
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                        <label class="sf-label">Issuance Type</label>
                        <select name="IssType" id="IssType" class="form-control" onchange="IssuanceType(this.value);">
                            <option value="">Select Issuance Type</option>
                            <option value="1">Issue With Job Order</option>
                            <option value="3">Issue Without Job Order</option>
                            <option value="4">Issue Damage Stock</option>

                        </select>
                    </div>
                </div>
                <div class="lineHeight">&nbsp;</div>
                <div class="row" id="ShowHideAll">
                    <?php echo Form::open(array('url' => 'pad/UpdateStockReturnDetail?m='.$m.'','id'=>'cashPaymentVoucherForm'));?>
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="IssuanceType" value="">
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                                                ...
                                                <input readonly type="text" class="form-control requiredField" placeholder="" name="is_no" id="is_no" value="{{strtoupper($is_no)}}" />
                                            </div>

                                            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                                                <label class="sf-label">ISSUANCE Date.</label>
                                                <span class="rflabelsteric"><strong>*</strong></span>
                                                <input type="date" class="form-control requiredField" max="<?php echo date('Y-m-d') ?>" name="iss_date_1" id="iss_date_1" value="<?php echo $issuance_date ?>" />
                                            </div>
                                            ...
                                            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12">
                                                ...
                                                <select class="form-control requiredField select2" name="region" id="region">
                                                    ...
                                                    @foreach(CommonHelper::get_all_regions() as $row)
                                                        <option value="{{$row->id}}" @if($row->id==$region) {{ "selected" }} @endif  >{{$row->region_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <?php
                                            CommonHelper::companyDatabaseConnection($m);
                                            $issuanceJoborder = DB::table('issuance')->where('status', '=', 1)->groupBy('joborder')->get();
                                            CommonHelper::reconnectMasterDatabase();
                                            ?>
                                            <div class="col-lg-3 col-md-3 col-sm-3 col-xs-12" id="IssueWithJobOrder2">
                                                <label class="sf-label">Job Order</label>
                                                <span class="rflabelsteric"><strong>*</strong></span>
                                                <select  name="joborder" id="joborder" class="form-control requiredField select2">
                                                    <option value="">Select Job Order</option>
                                                    <?php foreach($issuanceJoborder as $val):  ?>
                                                    <option value="<?= $val->joborder ?>" @if($val->joborder==$joborder) {{"selected"}} @endif > <?= $val->joborder ?> </option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="row">
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                ...
                                                <span class="rflabelsteric"><strong>*</strong></span>
                                                <textarea name="description_1" id="description_1" rows="4" cols="50" style="resize:none;" class="form-control requiredField">{{$description}}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                        <div class="table-responsive">
                                            <table id="buildyourform" class="table table-bordered">
                                                ...
                                                <?php
                                                $Counter=0;
                                                foreach($stock_return_data as $DataFil):
                                                $Counter++;
                                                ?>
                                                <tr id="removeDemandsRows_1_<?php echo $Counter?>">
                                                    <input type="hidden" name="issuanceDataSection_1[]" class="form-control requiredField" id="issuanceDataSection_{{$Counter}}" value="{{$Counter}}" />
                                                    <td>
                                                        <select name="category_id_1_{{$Counter}}" id="category_id_1_{{$Counter}}" onchange="subItemListLoadDepandentCategoryId(this.id,this.value)" class="form-control requiredField select2">
                                                            <?php echo PurchaseHelper::categoryList($_GET['m'],$DataFil->category);?>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <select onchange="get_detail(this.id,'<?php echo $Counter?>')"  name="sub_item_id_1_{{$Counter}}" id="sub_item_id_1_{{$Counter}}" class="form-control requiredField select2">
                                                            <?php echo PurchaseHelper::subItemList($_GET['m'],'',$DataFil->category,$DataFil->subitem);?>
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <input onkeyup="CurrentQty(this.id,'<?php echo $Counter?>')" step="0.01" type="number" name="qty_1_{{$Counter}}" id="qty_1_{{$Counter}}" class="form-control requiredField" value="<?php echo $DataFil->qty?>" />
                                                    </td>
                                                    <td><input class="form-control" readonly type="text" id="stock_qty_1_{{$Counter}}" name="stock_qty_1_{{$Counter}}"/> </td>
                                                    <td class="text-right"><input class="form-control" readonly type="text" id="current_qty_1_{{$Counter}}" name="current_qty_1_{{$Counter}}"/> </td>
                                                    <script>
                                                        $(document).ready(function(){
    ...
    <script>
        ...
        function get_detail(id,number) {
            ...
            $.ajax({
                ...
                success:function(data) {
                    ...
                    CurrentQty('qty_1_'+number,number);
        ...
        function CurrentQty(qty,num)
        {
            ...
            var current = parseFloat(stock)+parseFloat(quatity);
