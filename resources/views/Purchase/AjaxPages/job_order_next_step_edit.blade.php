...
$job_order_data = DB::table('job_order_data')->where('status',1)->where('job_order_id','=',$master_id)->get();
...
@section('content')
    ...
    <?php echo Form::open(array('url' => 'pad/addJobOrderNextStepUpdate','id'=>'JobOrderForm', 'enctype' => 'multipart/form-data')); ?>
    <div class="row">
                                        ...
                                        <div class="table-responsive">
                                            <div class="addMoreDemandsDetailRows_1" id="addMoreDemandsDetailRows_1">
                                                <table id="" class="table table-bordered">
                                                    ...
                                                    <?php
                                                    CommonHelper::companyDatabaseConnection($m);
                                                    $estimates = DB::table('estimate')->where('job_order_data_id','=',$Filter->job_order_data_id)->get();
                                                    CommonHelper::reconnectMasterDatabase();
                                                    ?>
                                                    ...
                                                    <tr class="text-center" id="RemoveTd<?php echo $Counter;?>">
                                                        <input type="hidden" name="estimate_id_<?php echo $Counter?>[]" value="<?php echo "$estimate->id"; ?>" >
                                                        <td>
                                                            1
                                                        </td>
                                                        <td>
                                                            <select onchange="get_data(this.id,'{{$Counter}}')" style="width: 100%" name="item_<?php echo $Counter?>[]" id="item_<?php echo $Counter?>" class="form-control select2">
                                                                ...
                                                                @foreach(CommonHelper::get_all_subitem() as $row)
                                                                    <option value="{{$row->id}}" <?php if($estimate->item==$row->id){ echo "selected"; } ?> >{{ucwords($row->sub_ic)}}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="UomDisp<?php echo $Counter?>[]" id="uom_<?php echo $Counter?>" readonly class="form-control">
                                                            <input type="hidden" name="uom_<?php echo $Counter?>[]" id="uom_<?php echo $Counter?>"  class="form-control">
                                                        </td>
                                                        <td><input type="text" name="stock_value_<?php echo $Counter?>[]" id="stock_value_<?php echo $Counter?>" readonly class="form-control"></td>
                                                        <td><input type="number" name="qty_<?php echo $Counter?>[]" id="qty_<?php echo $Counter?>" class="form-control" value="<?php echo $estimate->qty; ?>"></td>
