...
<div id="cost_center<?php echo $count ?>"  style="display: none;" class="col-lg-6 col-md-6 col-sm-6 col-xs-12 banks">

    <div class="row">
        <div id="dept_allocation<?php echo $count ?>"  class="col-lg-5 col-md-5 col-sm-5 col-xs-6 banks">
            <input checked type="checkbox" name="cost_center_check_box<?php echo $count ?>" value="1" id="cost_center_check_box<?php echo $count ?>" class="">Allow Null  </label>
        </div>
    ...
    <input type="hidden" name="cost_center_dept_hidden_amount<?php echo $count ?>" id="cost_center_dept_hidden_amount<?php echo $count ?>" value="<?php  if ($good_recipt_note->type==0):echo $net_amount; endif; ?>"/>

    <table  id="cost_center_table1" class="table table-bordered">

        <tbody class="cost_center_addrows<?php echo $count ?>" id="">
        <input type="hidden" name="" class="form-control requiredField" id="demandDataSection_1" value="1" />
        <tr>
            <td style="width: 40%">
                <select style="width: 100%" onchange="open_form_cost_center(this.id);check_repeatition(this.id,'<?php echo $count ?>')"  name="cost_center_department<?php echo $count ?>[]" id="cost_center_department_<?php echo $count ?>_1"
                        class="form-control select2 CostCenter cost_allocation<?php echo $count ?>">
            ...
            <td>
                <input onkeyup="cost_center_calculation_dept_item(this.id,'<?php echo $count ?>')" placeholder="Percentage" type="any" name="cost_center_percent<?php echo $count ?>[]" id="cost_center_percent_<?php echo $count ?>_1" class="form-control" />
            </td>
            <td class="text-right">

                <input onblur="add_cost_center('<?php echo $count ?>',this.id)"  onkeyup="cost_center_calculation_dept_item_amount(this.id,'<?php echo $count ?>');addclass(this.id,'<?php echo $count ?>_1')" class="cost_center_department_amount<?php echo $count ?> form-control" style="text-align: right" type="text" name="cost_center_department_amount<?php echo $count ?>[]" id="cost_center_department_amount_<?php echo $count ?>_1" class="form-control" />
            </td><!-->
        </tr>
        ...
        <tr>
            ...
            <td   style="text-align: right" id=""><input readonly type="text" value="0" id="cost_center_dept_amount<?php echo $count ?>" name="cost_center_dept_amount1"/> </td>
            <td colspan="1"><input style="text-align: right;font-size: 12px" readonly type="text" class="form-control" name="cost_center_total_dept<?php echo $count ?>" id="cost_center_total_dept<?php echo $count ?>"></td>
        </tr>
    </table>
</div>
<!--
...
<script>
    ...
    function cost_center_AddMore(number)
    {
        ...
        cost_center_allocation++;
        $(wrapper).append('<tr><td style="width: 40%"><select onchange="open_form_cost_center(this.id);check_repeatition(this.id,'+number+')" style="width: 100%;" name="cost_center_department'+number+'[]" id="cost_center_department_'+number+'_'+cost_center_allocation+'" class="form-control select2 CostCenter cost_allocation'+number+'">' +
                '<option value="">Select</option>   <option  value=""><b>Add New</b>  </option>@foreach(CommonHelper::get_all_cost_center() as $row)<option value="<?php echo $row->id ?>"><?php echo ucwords($row->name)?></option>@endforeach</select></td>' +
    ...
    function cost_center_heading_change(number)
    {
        ...
        $('#cost_center_dept_amount'+number).val(total)
